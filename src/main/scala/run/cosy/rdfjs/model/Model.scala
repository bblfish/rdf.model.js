...
/** Data Factory Functions
  ...
  */
class DataFactoryFncts(bnodeStart: BigInt = DataFactory.one) extends DataFactoryI:
...
end DataFactoryFncts

/** DataFactory that is easier to use from Scala
  */
class DataFactory(val dfFcts: DataFactoryFncts):
   import dfFcts.Nodes

   inline def namedNode(value: String): NamedNode = dfFcts.namedNode(value)

   inline def literal(value: String): Literal = dfFcts.literal(value, undefined)

   inline def literal(value: String, lang: String): Literal = dfFcts.literal(value, lang)

   inline def literal(value: String, dataType: NamedNode): Literal = dfFcts.literal(value, dataType)

   inline def variable(value: String): Variable = dfFcts.variable(value)

   inline def blankNode(): BlankNode = dfFcts.blankNode(undefined)

   inline def blankNode(label: String): BlankNode =
     dfFcts.blankNode(label)

   inline def quad(
       subject: Quad.Subject,
       rel: Quad.Predicate,
       obj: Quad.Object
   ): Quad = dfFcts.quad(subject, rel, obj, undefined)
   ...
   /** Required by rdflib.js which states: Generates a uniquely identifiably *idempotent* string for
     ...
     *   Use this to associate data with a term in an object { obj[id(term)] = "myData" }
     */
   inline def id(term: Nodes): String = dfFcts.id(term)
...
object DataFactory:
   ...
   @JSExport("create")
   def apply(): DataFactory = new DataFactory(new DataFactoryFncts(one))
